...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("input", async (event) => {
    ...
    const skillName =
      spaceIdx === -1 ? event.text.slice(7) : event.text.slice(7, spaceIdx);
    ...
    if (!allowsBash(fm["allowed-tools"]))
      return { action: "continue" as const };
  ...
  pi.on("tool_result", async (event) => {
    ...
    const textPiece = event.content?.find(
      (c: any) =>
        c.type === "text" && typeof c.text === "string" && PATTERN.test(c.text),
    ...
    return {
      content: event.content!.map((c: any) =>
        c === textPiece ? { type: "text", text: interpolated } : c,
      ),
    };
  });
}
