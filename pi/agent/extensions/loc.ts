...
function languageFor(file: string) {
  ...
  if (EXCLUDED_SUFFIXES.some((suffix) => file.endsWith(suffix)))
    return undefined;
  return (
    LANGUAGE_BY_BASENAME[basename] ??
    LANGUAGE_BY_EXTENSION[path.extname(file).toLowerCase()]
  );
}
  ...
  return [
    ...
    ...rows.map((row) => {
      const percent =
        totalLines === 0
          ? "0.0%"
          : `${((row.lines / totalLines) * 100).toFixed(1)}%`;
...
async function buildLocTable(cwd: string) {
  ...
  const rows = [...totals.values()].sort(
    (a, b) => b.lines - a.lines || a.language.localeCompare(b.language),
  );
  const table =
    rows.length > 0 ? toMarkdown(rows) : "No tracked source files found.";
  const note =
    skippedUnreadable > 0
...
export default function (pi: ExtensionAPI) {
  pi.registerCommand("loc", {
    description:
      "Show tracked source-code lines by language, largest to smallest",
    handler: async (_args, ctx) => {
      ...
      try {
        ...
        ctx.ui.notify(
          `Failed to calculate LOC: ${error instanceof Error ? error.message : String(error)}`,
          "error",
        );
      }
    },
