...
function fallowArgs(params: FallowParams) {
  ...
  const args: string[] = command === "full" ? [] : [command];
  args.push(...(params.args ?? []));

  if (params.json ?? true) {
    const hasFormat = args.some(
      (arg, index) =>
        arg === "--format" ||
...
function summarizeJson(stdout: string) {
  try {
    ...
    if (typeof parsed.verdict === "string")
      bits.push(`verdict=${parsed.verdict}`);
    if (typeof parsed.score === "number") bits.push(`score=${parsed.score}`);
    if (typeof parsed.issue_count === "number")
      bits.push(`issues=${parsed.issue_count}`);
    if (typeof parsed.issues === "object" && Array.isArray(parsed.issues))
      bits.push(`issues=${parsed.issues.length}`);
    return bits.join(" ");
  } catch {
    return "";
  }
...
export default function (pi: ExtensionAPI) {
    ...
    parameters: Type.Object({
      command: Type.Optional(
        Type.Union(
          ...
          {
            description:
              "Fallow subcommand. `full` runs `fallow` without a subcommand.",
          },
        ),
      ),
      args: Type.Optional(
        Type.Array(Type.String(), {
          description:
            'Additional CLI args, e.g. ["--changed-since", "main"] or ["--dry-run"].',
        }),
      ),
      cwd: Type.Optional(
        Type.String({
          description:
            "Working directory. Defaults to the current pi process cwd.",
        }),
      ),
      json: Type.Optional(
        Type.Boolean({
          description:
            "Append `--format json` when no format is supplied. Defaults to true.",
        }),
      ),
    }),
    async execute(_toolCallId, params, signal) {
      const result = await runFallow(
        params as FallowParams,
  ...
  pi.registerCommand("fallow", {
    description:
      "Run `fallow audit --format json` and send the result into the conversation",
    handler: async (args, ctx) => {
      await ctx.waitForIdle();
      const cliArgs = args.trim() ? args.trim().split(/\s+/) : [];
      const result = await runFallow(
        { command: "audit", args: cliArgs, json: true },
